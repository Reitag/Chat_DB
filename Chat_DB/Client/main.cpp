...
#include "../Chat_client/chat/chat.h"
	...
	while (open)
	{
		...
		case '2':
		{
			...
			if (chat.Chat::Connection::receiveResultFromServer() == true)
			{
				...
				while (open2)
				{
					std::cout << "Выберите действие:" << std::endl << "1.[Написать сообщение] 2.[Показать сообщение] 3.[Сменить пароль] 4.[Выйти из аккаунта]" << std::endl;
